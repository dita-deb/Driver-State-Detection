...
def main():
    ...
    args.input_dir = r"C:\Users\mc2di\project_1\Driver-State-Detection\output_images\Ramesh_Video_to_Images_5_FPS"
    ...
    # Initialize Scorer before the loop, so it's accessible
    Scorer = AttScorer(
        t_now=prev_time,  # Initialize with the initial time
    ...
    for image_file in image_files:
        ...
        # Calculate the current time and the time delta to measure FPS
        t_now = time.perf_counter()
        delta_time = t_now - prev_time
        if delta_time > 0:
            fps = 1.0 / delta_time  # Calculate frames per second
        prev_time = t_now
